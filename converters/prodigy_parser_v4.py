...
class ProdigyParserV4():
    supported_versions = ['4.47', '4.63', '4.73', '4.75']
    ...
    def __init__(self):
        ...
        self.measurement_types = ['XPS','UPS']
    ...
    def _getSpectrumMetadataFromSQL(self):
        ...
        for spectrum in self.spectra: 
            spec_id = self._getSQLNodeID(spectrum['spectrum_id'])
            self.con=sqlite3.connect(self.sql_connection)
            cur=self.con.cursor()
            query = 'SELECT * FROM Spectrum WHERE Node="{}"'.format(spec_id)
            cur.execute(query)
            results = cur.fetchall()
            if len(results) != 0:
                results = results[0]
    ...
    def _checkEncoding(self, *args):
        """Check whether the binary data should be decoded float or double."""
        self.con=sqlite3.connect(self.sql_connection)
        cur=self.con.cursor()
        if args:
            print("raw Id " + str(args[0]))
            query = 'SELECT LENGTH(Data),ChunkSize FROM CountRateData WHERE RawId == {} LIMIT 1'.format(args[0])
        else:
...
if __name__ == '__main__': 
    ...
    file ='EX439_S718_Au in 25 mbar O2.sle'
...
#%%
    ...
    versions = []
    for file in files:
        sle.setCurrentFile(folder + file)
        versions += [{'version':sle.getSLEVersion(),
                      'file':file}]
    unique_versions = set([ver['version'] for ver in versions])
    
    examples = []
    for v in unique_versions:
        one_example = [item for item in versions if item['version'] == v][0]
